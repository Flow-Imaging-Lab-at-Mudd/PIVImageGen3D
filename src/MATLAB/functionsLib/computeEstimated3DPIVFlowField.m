...
function [U, V, W] = computeEstimated3DPIVFlowField(pivParameters, imageProperties, particleMap, flowField)
...
%   V v-component velocities matrix 
    U = zeros(particleMap.IAs(1), particleMap.IAs(2), particleMap.IAs(3));
    V = zeros(particleMap.IAs(1), particleMap.IAs(2), particleMap.IAs(3));
    W = zeros(particleMap.IAs(1), particleMap.IAs(2), particleMap.IAs(3));
    ...
    for IAi = 2:particleMap.IAs(1)+1
        ...
        for IAj = 2:particleMap.IAs(2)+1
            ...
            for IAk = 2:particleMap.IAs(3)+1
                z0 = double(IAk - 2) * double(pivParameters.lastWindow(3)) + ...
                double(pivParameters.lastWindow(3))/2.0 + double(imageProperties.marginsZ)/2.0;

            
            [x1, y1, z1] = flowField.computeDisplacementAtImagePosition(x0, y0, z0);
            U(IAi-1, IAj-1, IAk-1) = x1 - x0;
            V(IAi-1, IAj-1, IAk-1) = y1 - y0;
            W(IAi-1, IAj-1, IAk-1) = z1 - z0;
        end
    end
end
